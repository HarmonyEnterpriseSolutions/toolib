...
class ParserDescription(yappsrt.Parser):
    ...
    def Options(self, _parent=None):
        ...
        opt = {}
        while self._peek('"option"', '"token"', '"ignore"', 'EOF', '"rule"') == '"option"':
            self._scan('"option"')
            self._scan('":"')
            Str = self.Str(_context)
            opt[Str] = 1
        if self._peek() not in ['"option"', '"token"', '"ignore"', 'EOF', '"rule"']:
            raise yappsrt.SyntaxError(self._scanner, charpos=self._scanner.get_prev_char_pos(), context=_context, msg='Need one of ' + ', '.join(['"option"', '"token"', '"ignore"', 'EOF', '"rule"']))
        return opt

    def Tokens(self, _parent=None):
        _context = self.Context(_parent, self._scanner, self._pos, 'Tokens', [])
        tok = []
        while self._peek('"token"', '"ignore"', 'EOF', '"rule"') in ['"token"', '"ignore"']:
            _token = self._peek('"token"', '"ignore"')
            if _token == '"token"':
                self._scan('"token"')
                ID = self._scan('ID')
                self._scan('":"')
        ...
        if self._peek() not in ['"token"', '"ignore"', 'EOF', '"rule"']:
            raise yappsrt.SyntaxError(self._scanner, charpos=self._scanner.get_prev_char_pos(), context=_context, msg='Need one of ' + ', '.join(['"token"', '"ignore"', 'EOF', '"rule"']))
        return tok

    def Rules(self, tokens, _parent=None):
        _context = self.Context(_parent, self._scanner, self._pos, 'Rules', [tokens])
        rul = []
        while self._peek('"rule"', 'EOF') == '"rule"':
        ...
        if self._peek() not in ['"rule"', 'EOF']:
            raise yappsrt.SyntaxError(self._scanner, charpos=self._scanner.get_prev_char_pos(), context=_context, msg='Need one of ' + ', '.join(['"rule"', 'EOF']))
        return rul
    ...
    def ClauseA(self, rule, tokens, _parent=None):
        ...
        v = [ClauseB]
        while self._peek('OR', 'RP', 'RB', '"rule"', 'EOF') == 'OR':
            OR = self._scan('OR')
            ClauseB = self.ClauseB(rule, tokens, _context)
            v.append(ClauseB)
        if self._peek() not in ['OR', 'RP', 'RB', '"rule"', 'EOF']:
            raise yappsrt.SyntaxError(self._scanner, charpos=self._scanner.get_prev_char_pos(), context=_context, msg='Need one of ' + ', '.join(['OR', 'RP', 'RB', '"rule"', 'EOF']))
    ...
    def ClauseB(self, rule, tokens, _parent=None):
        ...
        v = []
        while self._peek('STR', 'ID', 'LP', 'LB', 'STMT', 'OR', 'RP', 'RB', '"rule"', 'EOF') in ['STR', 'ID', 'LP', 'LB', 'STMT']:
            ClauseC = self.ClauseC(rule, tokens, _context)
            v.append(ClauseC)
        if self._peek() not in ['STR', 'ID', 'LP', 'LB', 'STMT', 'OR', 'RP', 'RB', '"rule"', 'EOF']:
            raise yappsrt.SyntaxError(self._scanner, charpos=self._scanner.get_prev_char_pos(), context=_context, msg='Need one of ' + ', '.join(['STR', 'ID', 'LP', 'LB', 'STMT', 'OR', 'RP', 'RB', '"rule"', 'EOF']))
    ...
    def ClauseC(self, rule, tokens, _parent=None):
        ...
        _token = self._peek('PLUS', 'STAR', 'QUEST', 'STR', 'ID', 'LP', 'LB', 'STMT', 'OR', 'RP', 'RB', '"rule"', 'EOF')
        if _token == 'PLUS':
    ...
    def ClauseD(self, rule, tokens, _parent=None):
        ...
        _token = self._peek('STR', 'ID', 'LP', 'LB', 'STMT')
        if _token == 'STR':
            STR = self._scan('STR')
            t = (STR, eval(STR,{},{}))
    ...
    def OptParam(self, _parent=None):
        ...
        _token = self._peek('ATTR', '":"', 'PLUS', 'STAR', 'QUEST', 'STR', 'ID', 'LP', 'LB', 'STMT', 'OR', 'RP', 'RB', '"rule"', 'EOF')
        if _token == 'ATTR':
