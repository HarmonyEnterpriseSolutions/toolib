...
class ParseError(Exception):
    """
    Any scanner exception
    """
    def __init__(self, scanner):
        Exception.__init__(self)
        self.scanner = scanner

    def printDescription(self, out):
        raise NotImplementedError, "abstract"


class SyntaxError(ParseError):
    """When we run into an unexpected token, this is the exception to use"""
    def __init__(self, scanner, charpos=-1, msg="Bad Token", context=None):
        ParseError.__init__(self, scanner)
    ...
    def printDescription(self, out=None):
        """
        ...
        column_number = self.scanner.get_column_number()
        print >>out, '%d:%d: %s' % (line_number, column_number, self.msg)

        context = self.context
        if not context:
            print_line_with_pointer(self.scanner.input, self.charpos, out)
        
        while context:
            # TODO: add line number
            print >>out, 'while parsing %s%s:' % (context.rule, tuple(context.args))
            print_line_with_pointer(self.scanner.input, context.scanner.get_prev_char_pos(context.tokenpos), out)
            context = context.parent

        print >>out, 'Stopped around here:'
        print >>out, self.scanner


class NoMoreTokens(ParseError):
    """Another exception object, for when we run out of tokens"""

    def printDescription(self, out=None):
        if out is None:	out = sys.stderr
        print >>out, 'Could not complete parsing; stopped around here:'
        print >>out, self.scanner
    ...
    def scan(self, restrict):
        ...
        while 1:
            ...
            if best_pat == '(error)' and best_match < 0:
                ...
                raise SyntaxError(self, self.pos, msg)
...
class Parser:
    ...
    def _scan(self, type):
        ...
        if tok[2] != type:
            raise SyntaxError(self._scanner, tok[0], 'Trying to find '+type+' :'+ ' ,'.join(self._scanner.restrictions[self._pos]))
...
def print_line_with_pointer(text, p, out):
...
def print_error(input, err, scanner):
    """DEPRECATED, remains for compatibility. Use err.printDescription()"""
    err.printDescription()
